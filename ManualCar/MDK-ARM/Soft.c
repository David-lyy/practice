...
uint16_t get_sbus[25] = {0};
uint8_t sbus_data[16] = {0};
fp32 set_speed[4] = {0};
...
//接收遥控器信号并解析映射为速度的所需要的转子速度值
void cul_rpm(uint16_t *channels, fp32 *set_speed) /*0 AND 1 L || 2 AND 3 R*/
{
  fp32 rspeed1 = channels[0] - 1020.0f;
  fp32 rspeed2 = channels[1] - 1020.0f;
  fp32 gap = 0.0f;
  ...
  if ((channels[0] > 360) && (channels[0] < 1680))
  {
    fp32 base_speed = rspeed1 * (MAX_SPEED / 1020.0f);
    // 限制转速范围，避免溢出
